...
# ==========================================================
# ADMIN — ATHENA BROWSER
# ==========================================================

@app.route('/api/admin/athena/tables', methods=['GET'])
@api_login_required
@role_required('Admin')
...
@app.route('/api/admin/athena/query', methods=['POST'])
@api_login_required
@role_required('Admin')
def admin_athena_query():
    """Preview rows from an Athena table with optional column filter."""
    data       = request.get_json()
    table      = data.get('table', '').strip()
    page       = int(data.get('page', 1))
    limit      = min(int(data.get('limit', 50)), 200)
    search     = data.get('search', '').strip()
    col_filter = data.get('col_filter', '').strip()

    if not table:
        return jsonify({'error': 'table required'}), 400

    # Only allow plain identifiers (letters, digits, underscores)
    import re as _re
    ...
    try:
        ...
        cnt_df = wr.athena.read_sql_query(
            sql=count_sql,
            database=ATHENA_DATABASE,
            s3_output=S3_STAGING_DIR,
            boto3_session=aws_session,
        ...
        total = int(cnt_df['cnt'].iloc[0])

        data_sql = f'SELECT * FROM "{table}" {where} OFFSET {offset} LIMIT {limit}'
        df = wr.athena.read_sql_query(
            sql=data_sql,
            database=ATHENA_DATABASE,
            s3_output=S3_STAGING_DIR,
        ...
        return jsonify({'columns': columns, 'rows': rows, 'total': total, 'page': page, 'limit': limit})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
S3_ADMIN_BUCKET = 'neo-advanced-analytics'

@app.route('/api/admin/s3/browse', methods=['GET'])
@api_login_required
@role_required('Admin')
def admin_s3_browse():
    """List objects and common prefixes (folders) at a given S3 prefix."""
    prefix = request.args.get('prefix', '')
    try:
        s3 = aws_session.client('s3')
        paginator = s3.get_paginator('list_objects_v2')
        pages = paginator.paginate(
            Bucket=S3_ADMIN_BUCKET,
            Prefix=prefix,
            Delimiter='/',
        ...
        for page in pages:
            for cp in page.get('CommonPrefixes') or []:
                folders.append({'name': cp['Prefix'][len(prefix):], 'prefix': cp['Prefix']})
            for obj in page.get('Contents') or []:
                ...
                files.append({
                    ...
                    'previewable': name.lower().endswith(('.csv', '.json', '.geojson', '.tsv', '.txt'))
                })
...
@app.route('/api/admin/s3/preview', methods=['POST'])
@api_login_required
@role_required('Admin')
def admin_s3_preview():
    """Return the first N rows of a CSV/JSON/text file from S3."""
    data = request.get_json()
    ...
    try:
        s3  = aws_session.client('s3')
        obj = s3.get_object(Bucket=S3_ADMIN_BUCKET, Key=key)
        body = obj['Body'].read()

        if ext in ('csv', 'tsv'):
            import io
            sep = '\t' if ext == 'tsv' else ','
            df  = pd.read_csv(io.BytesIO(body), sep=sep, nrows=200, dtype=str)
            ...
            # For GeoJSON, summarise features instead of dumping everything
            if isinstance(parsed, dict) and parsed.get('type') == 'FeatureCollection':
                features = parsed.get('features', [])
                sample = features[:50]
                props_keys = list(sample[0]['properties'].keys()) if sample and sample[0].get('properties') else []
                rows = [f['properties'] for f in sample]
                return jsonify({
                    'type': 'table',
                    'columns': props_keys,
                    'rows': rows,
                    'total_shown': len(sample),
                    'note': f'GeoJSON — showing first {len(sample)} of {len(features)} features (properties only)'
                })
            # Regular JSON array
            if isinstance(parsed, list):
                ...
                if sample and isinstance(sample[0], dict):
                    ...
                    return jsonify({'type': 'table', 'columns': cols, 'rows': [dict(r) for r in sample], 'total_shown': len(sample)})
            return jsonify({'type': 'raw', 'content': text[:8000]})
...
@role_required('Admin')
def admin_db_tables():
    """Return all user-facing tables with row counts."""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
            ...
            for tbl in tables:
                ...
                result.append({'table': tbl, 'row_count': count, 'columns': columns})
        return jsonify(result)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
@role_required('Admin')
def admin_db_query():
    """Execute a safe read-only query against the Postgres DB."""
    data = request.get_json()
    table  = data.get('table', '').strip()
    page   = int(data.get('page', 1))
    limit  = min(int(data.get('limit', 50)), 200)
    ...
    # Whitelist: only tables that actually exist in public schema
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
            ...
            # Build safe query – column names are not parameterised, but table is whitelisted above
            offset = (page - 1) * limit
            where_clause = ''
            params: list = []

            if search and col_filter:
                cursor.execute("""
                    SELECT column_name FROM information_schema.columns
                    WHERE table_schema='public' AND table_name=%s AND column_name=%s
                """, (table, col_filter))
                if cursor.fetchone():
                    where_clause = f'WHERE CAST("{col_filter}" AS TEXT) ILIKE %s'
            ...
            data_sql = f'SELECT * FROM "{table}" {where_clause} ORDER BY 1 LIMIT %s OFFSET %s'
...
@app.route('/download/s3-admin')
@api_login_required
@role_required('Admin')
def download_s3_admin():
    """Generate a presigned download URL for any S3 object (Admin only)."""
    ...
    try:
        ...
        url = s3_client.generate_presigned_url(
            'get_object',
            Params={'Bucket': S3_ADMIN_BUCKET, 'Key': key},
            ExpiresIn=300
        )
        return redirect(url)
    except Exception as e:
